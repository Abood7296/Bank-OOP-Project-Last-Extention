...
private:
    ...
    static clsUser _ConvertLineToUserObject(string Line, string Separator = "#//#")
    {
        ...
        return clsUser(enMode::UpdateMode, vUserData[0], vUserData[1], vUserData[2],
            vUserData[3], vUserData[4], clsUtil::EncryptDecryptPassword(vUserData[5], 'X'), stoi(vUserData[6]));
    }
    ...
    static string _ConvertUserObjectToLine(clsUser User, string Separator = "#//#")
    {
        ...
        UserRecord += clsUtil::EncryptDecryptPassword(User.Password,'X') + Separator;
    ...
    static vector <stLoginRegisterRecord> _LoadRegistersLoginFromFile()
    {
    ...
    {
        string LoginRecord = "";
        LoginRecord += clsDate::GetSystemDateString() + clsDate::GetSystemTimeString() + Separator;
        LoginRecord += UserName + Separator;
        LoginRecord += clsUtil::EncryptDecryptPassword(Password,'X') + Separator;
...
public:
    ...
    static bool IsCorrectPassword(string UserName, string Password)
    {
    ...
    {
		return _LoadRegistersLoginFromFile();
    }

    static vector <clsUser> GetUsersListByPermission(int Permissions)
    {
        vector <clsUser> vAllUsers = _LoadUsersDataFromFile();
        vector <clsUser> vUsers;
        for (clsUser U : vAllUsers)
        {
            if ((U.Permissions & Permissions) == Permissions)
            {
                vUsers.push_back(U);
            }
        }
